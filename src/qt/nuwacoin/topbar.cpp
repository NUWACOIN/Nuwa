...
#include "qt/nuwacoin/topbar.h"
#include "qt/nuwacoin/forms/ui_topbar.h"
#include "qt/nuwacoin/lockunlock.h"
#include "qt/nuwacoin/qtutils.h"
#include "qt/nuwacoin/receivedialog.h"
#include "qt/nuwacoin/loadingdialog.h"
...
TopBar::TopBar(NuwaCoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopNac}, "amount-small-topbar-main");
    setCssProperty({ui->labelAmountNac}, "amount-topbar");
    setCssProperty({ui->labelPendingNac, ui->labelImmatureNac}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("Nüwa Coin")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // NAC Total
    QString totalNac = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);

    // NAC
    // Top
    ui->labelAmountTopNac->setText(totalNac);
    // Expanded
    ui->labelAmountNac->setText(totalNac);
    ui->labelPendingNac->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureNac->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
