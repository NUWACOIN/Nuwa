...
#include "qt/nuwacoin/forms/ui_send.h"
#include "qt/nuwacoin/addnewcontactdialog.h"
#include "qt/nuwacoin/qtutils.h"
#include "qt/nuwacoin/sendchangeaddressdialog.h"
#include "qt/nuwacoin/optionbutton.h"
#include "qt/nuwacoin/sendconfirmdialog.h"
#include "qt/nuwacoin/myaddressrow.h"
#include "qt/nuwacoin/guitransactionsutils.h"
...
SendWidget::SendWidget(NuwaCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-nac");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination nacAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(nacAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(nacAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
