...
#include "qt/nuwacoin/forms/ui_snackbar.h"
#include "qt/nuwacoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(NuwaCoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &NuwaCoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
