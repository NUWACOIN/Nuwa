...
#include "qt/nuwacoin/forms/ui_addresseswidget.h"
#include "qt/nuwacoin/addresslabelrow.h"
#include "qt/nuwacoin/addnewaddressdialog.h"
#include "qt/nuwacoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(NuwaCoinGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination nacAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(nacAdd, label.toUtf8().constData(),
