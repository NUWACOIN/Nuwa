...
#include "qt/nuwacoin/dashboardwidget.h"
#include "qt/nuwacoin/forms/ui_dashboardwidget.h"
#include "qt/nuwacoin/sendconfirmdialog.h"
#include "qt/nuwacoin/txrow.h"
...
DashboardWidget::DashboardWidget(NuwaCoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	setCssProperty(ui->labelSquareNac, "square-chart-nac");
	setCssProperty(ui->labelNac, "text-chart-nac");
	...
	setCssProperty(ui->labelAmountNac, "text-stake-nac-disable");
	...
	if (window)
		connect(window, &NuwaCoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
	if (walletModel && walletModel->getOptionsModel()) {
		...
		connect(ui->pushImgEmpty, &QPushButton::clicked, window, &NuwaCoinGUI::openFAQ);
		connect(ui->btnHowTo, &QPushButton::clicked, window, &NuwaCoinGUI::openFAQ);
	...
	// update the display unit, to not use the default ("NAC")
	updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
	...
	chartData->amountsByCache = getAmountBy(); // pair NAC
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData->amountsByCache.contains(num)) {
			...
			nac = (pair.first != 0) ? pair.first / 100000000 : 0;
			chartData->totalNac += pair.first;
		}

		chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesNac.append(nac);

		int max = std::max(nac, nac);
...
void DashboardWidget::onChartRefreshed()
{
	...
	set0->setColor(QColor(185, 129, 30));
	...
	set0->append(chartData->valuesNac);

	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalNac > 0) {
		setCssProperty(ui->labelAmountNac, "text-stake-nac");
	}
	else {
		setCssProperty(ui->labelAmountNac, "text-stake-nac-disable");
	}
	forceUpdateStyle({ ui->labelAmountNac });
	ui->labelAmountNac->setText(GUIUtil::formatBalance(chartData->totalNac, nDisplayUnit));
