...
#include "qt/nuwacoin/splash.h"
#include "qt/nuwacoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("N\u00FCwa Coin", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in nuwacoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in nuwacoin.qrc)
...
/** Class encapsulating NuwaCoin startup and shutdown.
...
/** Main NuwaCoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (NuwaCoinGUI)
...
{
    window = new NuwaCoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &NuwaCoinGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &NuwaCoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(NuwaCoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(NuwaCoinGUI::DEFAULT_WALLET);
        ...
        // NuwaCoin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NuwaCoinGUI::handlePaymentRequest);
        connect(window, &NuwaCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. NuwaCoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(nuwacoin_locale);
    Q_INIT_RESOURCE(nuwacoin);
    ...
    /// 6. Determine availability of data directory and parse nuwacoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("N\u00FCwa Coin"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("N\u00FCwa Coin"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // nuwacoin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("N\u00FCwa Coin didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
