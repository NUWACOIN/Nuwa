...
bool CNacStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CNacStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CNacStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CNacStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CNacStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CNacStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CNacStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CNacStake::GetUniqueness() const
{
    //The unique identifier for a NAC stake is the outpoint
...
CBlockIndex* CNacStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CNacStake::ContextCheck(int nHeight, uint32_t nTime)
{
