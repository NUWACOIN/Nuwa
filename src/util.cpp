...
const char * const NuwaCoin_CONF_FILENAME = "nuwacoin.conf";
const char * const NuwaCoin_PID_FILENAME = "nuwacoin.pid";
const char * const NuwaCoin_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\NuwaCoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\NuwaCoin
// Mac: ~/Library/Application Support/nuwacoin
// Unix: ~/.nuwacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NuwaCoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NuwaCoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NuwaCoinParams
    // Mac: ~/Library/Application Support/NuwaCoinParams
    // Unix: ~/.nuwacoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NuwaCoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "NuwaCoinParams";
#else
    // Unix
    return pathRet / ".nuwacoin-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", NuwaCoin_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", NuwaCoin_MASTERNODE_CONF_FILENAME));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty nuwacoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override nuwacoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", NuwaCoin_PID_FILENAME));
