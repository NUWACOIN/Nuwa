...
#if defined(NDEBUG)
#error "NuwaCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in unac) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("nuwacoin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> nacInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                nacInputs.push_back(stakeIn);
        }
        const bool hasNACInputs = !nacInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasNACInputs) {
                    ...
                    for (const CTxIn& nacIn : nacInputs)
                        if(nacIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasNACInputs) {
                            for (const CTxIn& stakeIn : nacInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // NuwaCoin: We use certain sporks during IBD, so check to see if they are
