...
from test_framework.test_framework import NuwaCoinTestFramework
...
class ReorgStakeTest(NuwaCoinTestFramework):
    ...
    def check_money_supply(self, expected_bc, expected_znac):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected NAC and zNAC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_bc))
            for denom in node['zNACsupply']:
                assert_equal(node['zNACsupply'][denom], DecimalAmt(expected_znac[denom]))
    ...
    def run_test(self):
        ...
        # Check NAC and zNAC supply at the beginning
        # ------------------------------------------
        # zNAC supply: 2 coins for each denomination
        expected_znac_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # NAC supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_znac_supply)
        ...
        # Verify that NAC and zNAC supplies were properly updated after the spends and reorgs
        self.log.info("Check NAC and zNAC supply...")
        ...
        expected_znac_supply[str(spent_coin_0)] -= spent_coin_0
        expected_znac_supply[str(spent_coin_1)] -= spent_coin_1
        expected_znac_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_znac_supply)
