...
class ZerocoinSpendTest(NuwaCoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, znac_bal, bc_bal):
            znac_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], znac_bal)
            bc_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], bc_bal)
            return znac_bal, bc_bal
        ...
        znac_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(znac_balance, 6666)
        ...
        znac_balance, balance = check_balances(denom_2, znac_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        znac_balance, balance = check_balances(denom_3, znac_balance, balance)
        ...
        _, _ = check_balances(denom_1, znac_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
